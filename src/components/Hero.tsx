      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-32 text-center">
        ...
        <div className="flex justify-center mb-8 opacity-0 animate-fade-in-up">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(217,70,239,0.8) 0%, rgba(139,92,246,0.8) 100%)",
            }}
          >
        ...
        {/* Name */}
        <h1 className="text-6xl md:text-8xl font-bold text-zinc-100 mb-4 tracking-tight leading-none opacity-0 animate-fade-in-up animate-delay-200">
          Rudy <span className="text-fuchsia-400">Quinternet</span>
        </h1>

        {/* Title */}
        <p className="text-2xl md:text-3xl text-zinc-400 mb-4 font-light opacity-0 animate-fade-in-up animate-delay-300">
          Software Engineer
        </p>

        {/* Tagline */}
        <p className="text-zinc-500 text-base md:text-lg mb-10 max-w-md mx-auto leading-relaxed opacity-0 animate-fade-in-up animate-delay-300">
          Netherlands &middot; Full-stack &middot; Open to relocation
        </p>

        {/* CTAs */}
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6 opacity-0 animate-fade-in-up animate-delay-400">
          <a
            href="#contact"
        ...
        {/* Social links */}
        <div className="flex items-center justify-center gap-3 mb-14 opacity-0 animate-fade-in-up animate-delay-400">
        ...
        {/* Stats */}
        <div className="inline-flex items-stretch border border-zinc-800 rounded-2xl overflow-hidden bg-zinc-900/50 opacity-0 animate-fade-in-up animate-delay-500">
          {stats.map((stat, i) => (
            <div
              key={stat.label}
              className={`flex flex-col items-center px-8 py-4 ${i < stats.length - 1 ? "border-r border-zinc-800" : ""}`}
            >
              <span className="text-2xl font-bold text-fuchsia-400 leading-none">{stat.value}</span>
              <span className="text-zinc-500 text-xs mt-1.5 tracking-wide">{stat.label}</span>
            </div>
          ))}
        </div>
      </div>
